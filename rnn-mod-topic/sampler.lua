#!/usr/bin/env th
--[[
rnn-mod-topic requires topic vectors to be created artificially.
...
for i,token in ipairs(x_t) do
  ...
  if vocab[token] then
    ...
    model:forward({input_x,
                   ...
                   utils.zero(utils.tocuda(adddim(adddim(e_t[1])))), -- we feed the same event vector without decaying it
...
for i=1,#seedfile['miniplans'] do
  ...
  repeat
    ...
    local probs = model:forward({prev_x, -- x_t
                                 ...
                                 utils.zero(utils.tocuda(adddim(adddim(prev_e)))), -- e_t
