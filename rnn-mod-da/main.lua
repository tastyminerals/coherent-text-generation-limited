#!/usr/bin/env th
-- rnn-mod-da -- event and topic gates model with domain adaptation
...
cmd:option("--saturate",100,"epoch at which linear decay of lr reaches minlr") -- set to 400 if not adam
...
cmd:option("--adagrad", false, "use ADAGRAD instead of SGD as optimizer")
...
opt.adagradconfig = loadstring(" return "..opt.adagradconfig)()
if opt.silent then table.print(opt) end
opt.id = opt.id == '' and "inscript_"..dlr.uniqueid() or opt.id
...
local x,x_valid,_ = dlr.load_data(opt.path,{opt.batchsize,1,1},'<eos>','[trnvldai]+.txt')
...
local w2v_train = dlr.load_w2v('../word_embeddings/word2vec_inscript.t7',x)
-- (domain adaptation) load pretrained word2vec embeddings for validation data
local w2v_valid = dlr.load_w2v('../word_embeddings/word2vec_inscript.t7',x_valid)

-- load training topic & event vectors
local dims1,dims2 = {x.total_tokens,maxsentlen,1},{x_valid.total_tokens,maxsentlen,1}
local topics_train,events_train = dlr.load_csv_vectors(opt.path,'train_vecs.csv',opt.batchsize,dims1)
local topics_valid,events_valid = dlr.load_csv_vectors(opt.path,'valid_vecs.csv',1,dims2)
...
local adagradconfig = {
  lr = opt.adagradconfig[1],
  lrd = opt.adagradconfig[2],
  wd = opt.adagradconfig[3],
}
...
while opt.maxepoch <= 0 or epoch <= opt.maxepoch do
  ...
  for xbatch,w2vbatch,evbatch,topbatch in utils.zip(xit,w2vtit,evit,topit) do
    ...
    elseif opt.adagrad then
      local _, loss = optim.adagrad(feval, params, adagradconfig)
    else
  ...
  if not opt.silent then
    ...
    if opt.meanNorm then
      print("Network 1 mean grad_param norm",opt.meanNorm)
    end
  end
  ...
  pastalog('rnn-mod-da', 'PPL (train)', ppl, epoch, 'http://localhost:8120/data')
  ...
  -- report via pastalog(modelName, seriesName, value, step, [url])
  pastalog('rnn-mod-da', 'PPL (val)', ppl, epoch, 'http://localhost:8120/data')
