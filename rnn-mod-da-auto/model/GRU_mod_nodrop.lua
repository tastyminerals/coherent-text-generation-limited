...
------------------------------------------------------------------------
require "model.AbstractRecurrent"
require "model.ConcatTable"
require "model.Sequential"
require "model.FlattenTable"
require "model.LookupTable"
require "model.Reshape"


local GRU_mod_nodrop, parent = torch.class('GRU_mod_nodrop', 'nn.AbstractRecurrent')

function GRU_mod_nodrop:__init(opt, rho, p, mono)
...
-------------------------- factory methods -----------------------------
function GRU_mod_nodrop:build_event_gate()
...
function GRU_mod_nodrop:buildModel()
  ...
  t3:add(nn.SelectTable(3)) -- topic gate

  t1:add(nn.Linear(self.inputSize, self.outputSize))
  t2:add(nn.LinearNoBias(self.outputSize, self.outputSize)) -- Wh(r * ht-1)
  --t3:add(nn.Linear(self.inputSize, self.outputSize)) -- Uh * topic gate
  t3:add(nn.Linear(self.outputSize, self.outputSize)) -- Uh * topic gate
  ...
  e0:add(nn.SelectTable(2)) -- events
  e0:add(nn.Linear(self.tgateSize, self.outputSize))
...
------------------------- forward backward -----------------------------
function GRU_mod_nodrop:updateOutput(input)
...
function GRU_mod_nodrop:_updateGradInput(input, gradOutput)
...
function GRU_mod_nodrop:_accGradParameters(input, gradOutput, scale)
