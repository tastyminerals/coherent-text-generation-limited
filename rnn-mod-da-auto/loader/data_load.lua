...
require "loader.SequenceVecLoader"
require "loader.SequenceWord2VecLoader"
...
function get_max_sent_length(sents)
  local max = 0
  for i=1,#sents do
...
function data_load.build_vocab(tokens, minfreq)
...
function data_load.text2tensor(tokens, vocab)
  local oov = vocab['<OOV>']
...
function data_load.load_tokens(dirname,batchsize,eos_delim,trainfiles)
  ...
  for i,set in ipairs(trainfiles) do
    local filepath
    ...
    local maxsent = get_max_sent_length(sents)
    data = stringx.replace(data, '\n', eos_delim)
    local tokens = stringx.split(data)
    print(string.format("%s, tokens: %d, max sentlen: %d",filepath,#tokens,maxsent))
    if string.match(set,"train") and not vocab then
      vocab, ivocab, wordfreq = data_load.build_vocab(tokens)
    end
    local tensor = data_load.text2tensor(tokens, vocab)
...
local counter = 1
function data_load.uniqueid(namespace, separator)
  function hostname()
...
-- convert vector representation to 1-hot
function evec2onehot(events)
  local dim = 174 -- total available vectors + 1 for no event vector
...
-- load .json vectors
function data_load.load_json_vectors(dir,evfname,topfname,batchsize,dims)
  local evfile = get_files(dir,evfname)
  local topfile = get_files(dir,topfname)
...
-- extract word2vec embedding for each corresponding data token and return sequence
function data_load.load_w2v(w2v_path,data)
...
-- load vectors from csv files <this function is not a csv parser!>
-- dims = {traindata.total_tokens,maxsentlen,last dim}
function data_load.load_csv_vectors(dir,vecfile,batchsize,dims)
  local csvpath = get_files(dir,vecfile)
  local events = {}
  local topics = {}
...
function data_load.w2v(w2v_path)
  local w2v = torch.load(w2v_path)
  local w2vecs = {}
